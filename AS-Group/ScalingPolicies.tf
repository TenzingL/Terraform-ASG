resource "aws_autoscaling_policy" "scale-out" {
    ...
    cooldown = 300 //Time after policy goes off until new alarm can go off
...
resource "aws_cloudwatch_metric_alarm" "high-cpu-usage" {
    ...
    statistic = "Average"
    alarm_description = "This metric sets off an alarm if CPU usage is over 40% for 30 consecutive seconds"
    dimensions = {
